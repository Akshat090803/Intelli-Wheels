...
//!----
  ...
  //   if (day < today) {
  //     return "past"; // Return string "past" for past dates
  //   }
  ...
  //   // 2. Disable if dealership is closed on this day
  //   if (!daySchedule || !daySchedule.isOpen) {
  //     return "closed"; // Return string "closed" for dealership closed days
  //   }

  //   // If the day is neither past nor closed, it's enabled
  //   return false; // Return false if the day is valid
  // };
...
//   if (day < today) {
//     modifiers.push("past");
//     modifiers.push("disabled"); // Also mark as 'disabled' for selection
//   }

//   // Get day of week and check dealership hours
//   const dayOfWeek = format(day, "EEEE").toUpperCase();
//   const daySchedule = dealership?.workingHours?.find(
//     (schedule) => schedule.dayOfWeek === dayOfWeek
//   );

//   // 2. Check if dealership is closed on this day
//   if (!daySchedule || !daySchedule.isOpen) {
//     modifiers.push("closed");
//     modifiers.push("disabled"); // Also mark as 'disabled' for selection
//   }

//   // If there are any modifiers, return them. Otherwise, return false (meaning not disabled)
...
// };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="md:col-span-1">
        <Card className="bg-footer">
          <CardContent className="p-6">
            ...
            <div className="mt-2 text-xl font-bold text-price">
              ${car.price.toLocaleString()}
            </div>

            <div className="mt-4 text-sm text-subhead">
                      ...
                      <Popover    open={openPop} onOpenChange={setOpenPop}>
                        ...
                        <PopoverContent className="w-auto p-0">
                          <Calendar
                            ...
                            initialFocus
                            className="bg-footer shadow-2xl backdrop-blur-3xl "
                            classNames={{
                              day_selected:"bg-red-500",
                              day_today:"bg-background shadow-2xl text-white",
                              day: "inline-flex h-8 w-8 items-center justify-center rounded-md text-sm transition-colors hover:bg-[#1e2d50] hover:text-white",
                             day_disabled: "line-through text-gray-400 decoration-gray-500 decoration-2 pointer-events-none cursor-not-allowed"
      ...
      mode="single"
      selected={field.value}
      // onSelect={field.onChange} // This will be handled by the popover close
      onSelect={(date) => {
            field.onChange(date); // Update the form field
            setOpenPop(false); // Close the popover after selection
       
          }}
      disabled={isDayDisabled}
      initialFocus
      className="bg-[#1e293b] p-3 rounded-lg shadow-xl border border-[#334155]" 
      classNames={{
       
        caption: "flex justify-between items-center py-2 relative text-white font-semibold text-lg",
        caption_label: "text-white text-lg",
        nav: "flex items-center space-x-1",
        nav_button: "h-9 w-9 bg-[#111827] text-white p-0 hover:bg-[#16223c] rounded-md", // Darker buttons
        ...
        day: "h-9 w-9 p-0 font-normal text-white aria-selected:opacity-100 rounded-md hover:bg-[#2d3b4f] transition-colors", 
      
        day_selected:
          "bg-blue-600 text-white hover:bg-blue-700 focus:bg-blue-700 rounded-md",
        day_today:
          "border border-blue-500 text-blue-500 font-semibold rounded-md bg-transparent", 
        day_disabled: "text-gray-500 line-through opacity-60 cursor-not-allowed",
        day_range_middle: "bg-blue-500 opacity-50", 
  ...
  // --- IMPORTANT CHANGE: Pass the function directly to disabled ---
  disabled={isDayDisabled}
  // --- IMPORTANT CHANGE: Define styles for your custom modifiers and the 'disabled' modifier ---
  modifiersClassNames={{
    past: "!text-red-400 !bg-red-900/20  opacity-70 cursor-not-allowed", // Style for dates marked as 'past'
    closed: "!bg-gray-800/50 opacity-60 line-through cursor-not-allowed", // Style for dates marked as 'closed'
    disabled: "pointer-events-none" // General style for ALL unselectable days (ensures they aren't clickable)
  }}
  initialFocus
  className="bg-[#1e293b] p-3 rounded-lg shadow-xl border border-[#334155]"
  classNames={{
    ...
    caption_label: "text-white text-lg",
    nav: "flex items-center space-x-1",
    nav_button: "h-9 w-9 bg-[#111827] text-white p-0 hover:bg-[#16223c] rounded-md",
                  ...
                  render={({ field }) => (
                    <div>
                      ...
                      >
                        ...
                        <SelectContent className="bg-footer shadow-2xl  backdrop-blur-3xl">
                          {availableTimeSlots.map((slot) => (
                            <SelectItem key={slot.id} value={slot.id} className="transition-all duration-75 focus:bg-[#16223c] ">
                              {slot.label}
                            </SelectItem>
                          ))}
                        </SelectContent>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Textarea
                      ...
                      className="min-h-24 placeholder:text-subhead focus:outline-none focus-visible:ring-0"
                    />
                  )}
                />
              </div>
      ...
      <Dialog open={showConfirmation} onOpenChange={setShowConfirmation}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-subhead">
            ...
            <div className="py-4">
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{dealership?.name || "Intelli Wheels"}</span>
                </div>
              </div>
